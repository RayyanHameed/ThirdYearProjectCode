...
def proj_l2_ball(p1, p2, alpha):
    nrm = np.sqrt(p1*p1 + p2*p2)
    # avoid division by zero; the max handles it but keep it clean
    scale = np.maximum(1.0, nrm / alpha)
    return p1 / scale, p2 / scale


def function_TV_denoising_CP(f, alpha, iter):
    (n, m) = f.shape

    # keep same pattern as your code: alpha as per-pixel map
    alpha = alpha * np.ones((n, m))
    max_its = iter

    L2 = 8  # same as you used
    ...
    for k in range(1, max_its + 1):
        ...
        # plain TV projection (replaces P_a_Huber(...))
        p1, p2 = proj_l2_ball(p1, p2, alpha)

        # primal update (same structure as yours)
